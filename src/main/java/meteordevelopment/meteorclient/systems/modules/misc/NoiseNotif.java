...
public class NoiseNotif extends Module {
    ...
    private final PointerBuffer soundFileFilters;
    ...
    public NoiseNotif() {
        ...
        soundFileFilters = BufferUtils.createPointerBuffer(1);
        ByteBuffer oggFilter = MemoryUtil.memASCII("*.ogg");
        soundFileFilters.put(oggFilter);
        soundFileFilters.rewind();
    }
    ...
    private void chooseCustomSoundFile() {
        String path = TinyFileDialogs.tinyfd_openFileDialog("Choose Sound File", null, soundFileFilters, "OGG files (*.ogg)", false);
    ...
    private void dispatch(ReaderRule rule, Text matchedText) {
        if (rule.notifySound) playSound(notificationSound());
