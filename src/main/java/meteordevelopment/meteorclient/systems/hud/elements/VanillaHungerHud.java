 ...
 * hunger/food icon row to this element's position/scale.
 */
public class VanillaHungerHud extends HudElement {
    public static final HudElementInfo<VanillaHungerHud> INFO = new HudElementInfo<>(Hud.VANILLA_GROUP, "vanilla-hunger", "Moves and scales the real hunger/food icon row - reported not working correctly, under investigation.", VanillaHungerHud::new);
    ...
    private final Setting<Integer> spacing = sgGeneral.add(new IntSetting.Builder()
        .name("spacing")
        .description("Distance between each food icon's own center, before Scale is applied (8 is vanilla's own spacing, which slightly overlaps each 9px icon sprite with the next).")
        .defaultValue(8)
        .onChanged(v -> calculateSize())
        .build()
    );
